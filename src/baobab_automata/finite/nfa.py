...
from .nfa_exceptions import ConversionError, InvalidNFAError, InvalidTransitionError, NFAError
...
class NFA(AbstractFiniteAutomaton):
    ...
    def union(self, other: 'NFA') -> 'NFA':
        ...
        new_transitions[(new_initial, 'epsilon')] = {f"nfa1_{self._initial_state}", f"nfa2_{other._initial_state}"}
        ...
        try:
            ...
            for (source, symbol), targets in self._transitions.items():
                ...
                if not targets.issubset(self._states):
                    return False
                if symbol not in self._alphabet and symbol != 'epsilon':  # 'epsilon' pour les transitions epsilon
                    return False
