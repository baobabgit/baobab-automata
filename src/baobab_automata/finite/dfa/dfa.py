...
class Match:
    """
    Classe représentant une correspondance trouvée par un automate.
    
    :param match: Le texte correspondant
    :type match: str
    :param start: Position de début de la correspondance
    :type start: int
    :param end: Position de fin de la correspondance
    :type end: int
    """
    
    def __init__(self, match: str, start: int, end: int):
        self.match = match
        self.start = start
        self.end = end
    ...
    def __repr__(self) -> str:
        return f"Match('{self.match}', {self.start}, {self.end})"
...
class DFA(AbstractFiniteAutomaton):
    ...
    def find_longest_match(self, text: str, start: int = 0) -> Optional["Match"]:
        """
        Trouve la correspondance la plus longue dans le texte à partir d'une position donnée.

        :param text: Texte dans lequel chercher
        :type text: str
        :param start: Position de départ dans le texte
        :type start: int
        :return: Objet Match contenant la correspondance trouvée ou None
        ...
        # Essayer de faire correspondre à partir de chaque position
        for i in range(start, len(text)):
            current_state = self._initial_state
            match_length = 0

            # Simuler l'automate à partir de la position i
            for j in range(i, len(text)):
                ...
                # Si on atteint un état final, c'est une correspondance valide
                if current_state in self._final_states:
                    if match_length > longest_length:
                        longest_match = Match(text[i:i + match_length], i, i + match_length)
