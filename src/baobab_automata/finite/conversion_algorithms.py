...
class ConversionStats:
    ...
    def add_conversion(self, source_type: str, target_type: str, time_taken: float) -> None:
        """
...
class ConversionAlgorithms:
    ...
    def _get_cache_key(self, automaton: AbstractFiniteAutomaton, target_type: str) -> str:
        """
    ...
    def validate_conversion(self, original: AbstractFiniteAutomaton, 
        ...
        try:
            ...
            test_words = [
                "",  # Mot vide
                "a", "b", "c",  # Mots simples
                "aa", "ab", "ba", "bb",  # Mots de longueur 2
                "aaa", "aab", "aba", "abb", "baa", "bab", "bba", "bbb",  # Mots de longueur 3
            ]
            
            # Filtrer les mots selon l'alphabet
            alphabet = original.alphabet.intersection(converted.alphabet)
            test_words = [word for word in test_words if all(c in alphabet for c in word)]
    ...
    def nfa_to_dfa(nfa: NFA) -> DFA:
        ...
        try:
            ...
            dfa_final_states = {
                state_names[state] for state in dfa_states
    ...
    def nfa_to_dfa_optimized(self, nfa: NFA) -> DFA:
        ...
        try:
            ...
            raise ConversionError(f"Error in optimized NFA to DFA conversion: {e}") from e
                ...
                for symbol in epsilon_nfa.alphabet:
                    ...
                    for closure_state in epsilon_closures[state]:
                        ...
                        if transition_key in epsilon_nfa._transitions:
                            direct_transitions.update(epsilon_nfa._transitions[transition_key])
    ...
    def epsilon_nfa_to_nfa_optimized(self, epsilon_nfa: EpsilonNFA) -> NFA:
        ...
        try:
            ...
            raise ConversionError(f"Error in optimized ε-NFA to NFA conversion: {e}") from e
    ...
    def epsilon_nfa_to_dfa(epsilon_nfa: EpsilonNFA) -> DFA:
        ...
        try:
            ...
            dfa_initial = frozenset(epsilon_nfa.epsilon_closure({epsilon_nfa.initial_state}))
    ...
    def epsilon_nfa_to_dfa_via_nfa(self, epsilon_nfa: EpsilonNFA) -> DFA:
        ...
        try:
            ...
            raise ConversionError(f"Error in ε-NFA to DFA conversion via NFA: {e}") from e
    ...
    def epsilon_nfa_to_dfa_optimized(self, epsilon_nfa: EpsilonNFA) -> DFA:
        ...
        try:
            ...
            raise ConversionError(f"Error in optimized ε-NFA to DFA conversion: {e}") from e
    ...
    def regex_to_automaton_optimized(self, regex: str, target_type: str = 'epsilon_nfa') -> AbstractFiniteAutomaton:
        ...
        try:
            ...
            raise ConversionError(f"Error in optimized regex to automaton conversion: {e}") from e
    ...
    def automaton_to_regex_optimized(self, automaton: AbstractFiniteAutomaton) -> str:
        ...
        try:
            ...
            # Enregistrer les statistiques
            conversion_time = time.time() - start_time
            self._stats.add_conversion(type(automaton).__name__, "regex", conversion_time)
            
            return regex
            
        except Exception as e:
            raise ConversionError(f"Error in optimized automaton to regex conversion: {e}") from e
                    ...
                    if k <= n:
                        R[k][i][j] = ConversionAlgorithms._combine_regex(
                            ...
                            ConversionAlgorithms._concatenate_regex(
                                R[k-1][i][k-1],
                                ConversionAlgorithms._kleene_star_regex(R[k-1][k-1][k-1]),
                                R[k-1][k-1][j]
                            )
                        )
                    else:
