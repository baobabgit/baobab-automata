...
class TestContextFreeGrammar:
    ...
    def test_grammar_get_terminals_used_in_productions(self):
        ...
        grammar = ContextFreeGrammar(
            variables={"S", "A"},
            terminals={"a", "b", "c"},
            productions={Production("S", ("A", "a")), Production("A", ("b",))},
            start_symbol="S",
        )
...
class TestGrammarParser:
    ...
    def test_analyze_recursion_complex(self):
        ...
        assert (
            "left_recursive_variables" in analysis or "recursive_variables" in analysis
        )
...
<grammar name="Test Grammar">
    ...
    def test_convert_to_binary_productions(self):
        ...
        grammar = ContextFreeGrammar(
            variables={"S"},
            terminals={"a", "b", "c"},
            productions={Production("S", ("a", "b", "c"))},  # Production ternaire
            start_symbol="S",
        )
    ...
    def test_validate_grammar_no_variables(self):
        ...
        with pytest.raises(
            ValueError, match="L'ensemble des variables ne peut pas être vide"
        ):
            ContextFreeGrammar(
                variables=set(), terminals={"a"}, productions=set(), start_symbol="S"
            )
    ...
    def test_validate_grammar_no_productions(self):
        ...
        with pytest.raises(
            ValueError, match="L'ensemble des productions ne peut pas être vide"
        ):
            ContextFreeGrammar(
                variables={"S"}, terminals={"a"}, productions=set(), start_symbol="S"
            )
    ...
    def test_validate_grammar_invalid_start_symbol(self):
        ...
        with pytest.raises(
            ValueError, match="Le symbole de départ doit être une variable"
        ):
            ContextFreeGrammar(
                variables={"S"},
                terminals={"a"},
