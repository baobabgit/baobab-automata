...
class TestOperationStats(unittest.TestCase):
    ...
    def test_import_from_dict_valid(self):
        ...
        data = {
            'history': [
                {'operation': 'union', 'time': 0.1, 'states': 10, 'transitions': 20, 'memory': 1000},
                {'operation': 'intersection', 'time': 0.2, 'states': 15, 'transitions': 30, 'memory': 1500}
            ]
        }
    ...
    def test_import_from_dict_resets_existing(self):
        ...
        data = {
            'history': [
                {'operation': 'new', 'time': 0.2, 'states': 15, 'transitions': 30, 'memory': 0}
            ]
        }
        self.stats.import_from_dict(data)
