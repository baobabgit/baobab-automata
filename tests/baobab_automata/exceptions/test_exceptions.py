...
class TestBaobabAutomataError:
    ...
    def test_exception_with_details(self):
        ...
        assert str(error) == "Test error message (Détails: {'key': 'value', 'number': 42})"
        assert error.message == "Test error message"
...
class TestInvalidTransitionError:
    ...
    def test_transition_error_with_states(self):
        ...
        assert str(error) == "Invalid transition (Détails: {'source_state': 'q0', 'target_state': 'q1'})"
...
class TestConversionError:
    ...
    def test_conversion_error_with_types(self):
        ...
        assert str(error) == "Conversion failed (Détails: {'source_type': 'nfa', 'target_type': 'dfa'})"
    ...
    def test_conversion_error_with_step(self):
        ...
        assert str(error) == "Conversion failed (Détails: {'conversion_step': 'determinization'})"
        assert error.source_type is None
        assert error.target_type is None
...
class TestRecognitionError:
    ...
    def test_recognition_error_with_step(self):
        ...
        assert str(error) == "Recognition failed (Détails: {'recognition_step': 'symbol_processing'})"
        assert error.word is None
        assert error.automaton_type is None
